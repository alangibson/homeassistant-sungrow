...
class SungrowModbusTcpClient(ModbusTcpClient):
    ...
    def __init__(self, priv_key=PRIV_KEY, *args, **kwargs):
        ModbusTcpClient.__init__(self, *args, **kwargs)
        self._fifo = bytes()
    ...
    def _getkey(self):
        if (self._key is None) or (self._key_date != date.today()):
            ...
            if (
                (len(self._pub_key) == 16)
                and (self._pub_key != NO_CRYPTO1)
                and (self._pub_key != NO_CRYPTO2)
            ):
                self._setup()
            else:
    ...
    def _send_cipher(self, request):
        ...
        return (
            ModbusTcpClient.send(self, encrypted_request) - len(crypto_header) - padding
        )
