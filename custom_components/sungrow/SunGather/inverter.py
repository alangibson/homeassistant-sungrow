...
class SungrowInverter:
    ...
    def __init__(self, config_inverter):
        ...
        self.registers_custom = [
            {"name": "export_to_grid", "unit": "W", "address": "vr001"},
    ...
    def connect(self):
        ...
        if self.client:
            logger.debug(f"Reusing client: {self.client}")
        else:
    ...
    def checkConnection(self):
        ...
        if self.client:
            if self.client.is_socket_open():
                logger.debug("Client session is still connected")
                return True
            else:
                logger.info("Connecting new session")
                return self.connect()
        else:
            logger.info(f"Client is not connected, attempting to reconnect")
            return self.connect()
    ...
    def configure_registers(self, registersfile):
        ...
        if self.inverter_config.get("model"):
            ...
            for register in registersfile["registers"][0]["read"]:
                if register.get("name") == "device_type_code":
                    ...
                    if self.load_registers(
                        register["type"], register["address"] - 1, 1
                    ):
        ...
        for register in registersfile["registers"][0]["read"]:
            if (
                register.get("level", 3) <= self.inverter_config.get("level")
                or self.inverter_config.get("level") == 3
            ):
                register["type"] = "read"
                register.pop("level")
                if register.get("smart_meter") and self.inverter_config.get(
                    "smart_meter"
                ):
                    register.pop("models")
                    self.registers.append(register)
                    ...
                    register.get("models")
                    and not self.inverter_config.get("level") == 3
                ):
        ...
        for register in registersfile["registers"][1]["hold"]:
            if (
                ...
                if register.get("smart_meter") and self.inverter_config.get(
                    ...
                    register.get("models")
                    and not self.inverter_config.get("level", 1) == 3
                ):
                    for supported_model in register.get("models"):
                        if supported_model == self.inverter_config.get("model"):
                            register.pop("models")
                ...
                if register_type == register["type"] and register["address"] == run:
                    ...
                    if register.get("datatype") == "U16":
                        ...
                        if register.get("mask"):
                            ...
                            register_value = (
                                1 if register_value & register.get("mask") != 0 else 0
                            )
                        ...
                        ):  # Anything greater than 32767 is a negative for 16bit
                        ...
                        u32_value = rr.registers[num + 1]
                        if register_value == 0xFFFF and (
                            u32_value == 0xFFFF or u32_value == 0x7FFF
                        ):
                            register_value = 0
                        elif (
                            u32_value >= 32767
                        ):  # Anything greater than 32767 is a negative
                            register_value = (
                        ...
                        logger.debug(
                            f"{register_name} of type {register['datatype']} read as {register_value}"
                        )
    ...
    def scrape(self):
        ...
        try:
            if not self.latest_scrape["load_power"]:
                self.latest_scrape["load_power"] = int(
                    self.latest_scrape.get("total_active_power")
            ...
            if self.latest_scrape.get("load_power_hybrid"):
                self.latest_scrape["load_power"] = self.latest_scrape[
                    "load_power_hybrid"
                ]
        ...
        try:
            if self.latest_scrape.get("start_stop"):
                if self.latest_scrape.get(
                    "start_stop", False
                ) == "Start" and self.latest_scrape.get("work_state_1", False).contains(
                    "Run"
                ):
