...
class SungrowInverterConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def validate_input(
        self, hass: HomeAssistant, config_inverter: dict = None
    ) -> dict[str, Any]:
        ...
        is_success, inverter = await hass.async_add_executor_job(
            connect_inverter(config_inverter)
        )
        # If we can't connect, set a value indicating this so we can tell the user
        logger.debug(f"validate_input inverter.connect() is_success={is_success}")
        if not is_success:
    ...
    async def async_step_user(self, user_input=None):
        ...
        if not user_input:  # Just show the modal form and return if no user input
            ...
            if not errors:
                ...
                logger.debug(
                    f"async_step_user calling async_create_entry with unique_id {unique_device_id}"
                )
